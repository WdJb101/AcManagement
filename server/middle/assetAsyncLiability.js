...
const assetAsyncLiability = tryCatch(async (req, res, next) => {
  const { ac_name, ac_id, transaction_name, transaction_by, debit, credit } =
    req.body;
  if (!["Asset", "Liability"].includes(ac_name)) {
    return next();
  }
  let findAc;
  if (ac_name === "Asset") {
    findAc = await Account.findOne({ ac_name: "Liability" });
  }
  if (ac_name === "Liability") {
    findAc = await Account.findOne({ ac_name: "Asset" });
  }
  if (!findAc) {
...
module.exports = assetAsyncLiability;
